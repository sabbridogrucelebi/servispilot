...
@section('content')
    <h1 class="text-2xl font-bold mb-6">Sefer Düzenle</h1>

    <form action="{{ route('trips.update', $trip) }}" method="POST" class="bg-white p-6 rounded shadow">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <select name="service_route_id" class="w-full border rounded px-3 py-2">
                    ...
                    @foreach($serviceRoutes as $route)
                        <option value="{{ $route->id }}" {{ old('service_route_id', $trip->service_route_id) == $route->id ? 'selected' : '' }}>
                            {{ $route->route_name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="date" name="trip_date" value="{{ old('trip_date', $trip->trip_date?->format('Y-m-d')) }}" class="w-full border rounded px-3 py-2">
            ...
            <div>
                ...
                <select name="trip_status" class="w-full border rounded px-3 py-2">
                    <option value="Yapıldı" {{ old('trip_status', $trip->trip_status) == 'Yapıldı' ? 'selected' : '' }}>Yapıldı</option>
                    <option value="Yapılmadı" {{ old('trip_status', $trip->trip_status) == 'Yapılmadı' ? 'selected' : '' }}>Yapılmadı</option>
                    <option value="Eksik" {{ old('trip_status', $trip->trip_status) == 'Eksik' ? 'selected' : '' }}>Eksik</option>
                    <option value="Mesai" {{ old('trip_status', $trip->trip_status) == 'Mesai' ? 'selected' : '' }}>Mesai</option>
                </select>
            </div>

            <div>
                <label class="block mb-1">Sefer Fiyatı</label>
                <input type="number" step="0.01" name="trip_price" value="{{ old('trip_price', $trip->trip_price) }}" class="w-full border rounded px-3 py-2">
            </div>
        </div>

        <div class="mt-4">
            <label class="block mb-1">Notlar</label>
            <textarea name="notes" rows="4" class="w-full border rounded px-3 py-2">{{ old('notes', $trip->notes) }}</textarea>
        </div>
