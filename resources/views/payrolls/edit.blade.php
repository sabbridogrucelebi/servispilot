...
@section('title', 'Maaş Kaydını Düzenle')
@section('subtitle', 'Personel hakediş bilgilerini güncelle')
                ...
                <div class="rounded-[32px] border border-slate-200 bg-white p-8 shadow-sm">
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        <div class="md:col-span-2">
                            <label class="mb-2 block text-sm font-bold text-slate-700">Personel / Şoför</label>
                            <div class="flex items-center gap-4 rounded-2xl bg-slate-50 p-4 border border-slate-100">
                                <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-blue-600 text-white text-lg font-black shadow-lg shadow-blue-200">
                                    {{ strtoupper(substr($payroll->driver?->full_name ?? 'P', 0, 1)) }}
                                </div>
                                <div>
                                    <div class="font-bold text-slate-900">{{ $payroll->driver?->full_name }}</div>
                                    <input type="hidden" name="driver_id" value="{{ $payroll->driver_id }}">
                                    <div class="text-xs text-slate-500">Mevcut personel kaydı değiştirilemez</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="month" name="period_month" value="{{ old('period_month', $payroll->period_month) }}" 
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="number" step="0.01" name="base_salary" id="base_salary" value="{{ old('base_salary', $payroll->base_salary) }}" 
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="number" step="0.01" name="extra_payment" id="extra_payment" value="{{ old('extra_payment', $payroll->extra_payment) }}" 
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="number" step="0.01" name="deduction" id="deduction" value="{{ old('deduction', $payroll->deduction) }}" 
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="number" step="0.01" name="advance_payment" id="advance_payment" value="{{ old('advance_payment', $payroll->advance_payment) }}" 
                                       class="calc-input w-full rounded-2xl border-rose-100 bg-rose-50/30 py-3 pl-4 pr-12 text-rose-700 focus:border-rose-500 focus:ring-rose-500" placeholder="0,00">
                                <span class="absolute inset-y-0 right-4 flex items-center text-rose-400 font-bold">₺</span>
                            </div>
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="notes" rows="3" class="w-full rounded-2xl border-slate-200 bg-slate-50/50 py-3 px-4 text-slate-900 focus:border-blue-500 focus:ring-blue-500" placeholder="Maaş detayları, prim sebebi vb.">{{ old('notes', $payroll->notes) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
