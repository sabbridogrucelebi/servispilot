...
<head>
    ...
    <style>
        ...
        .stat-card { flex: 1; background: #ffffff; border: 1px solid #e2e8f0; padding: 18px; border-radius: 20px; position: relative; overflow: hidden; }
        ...
        .total-card-pro { flex: 1; background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%); color: white; padding: 25px; border-radius: 32px; }
        .net-val-pro { font-family: 'Outfit', sans-serif; font-size: 34px; font-weight: 900; line-height: 1; display: block; }
...
<body onload="window.print()">
    @foreach($reports as $index => $data)
        @php
            ...
            $extraBonus = $ex ? (float)$ex->extra_bonus : 0; $extraNotes = $ex ? $ex->extra_notes : '';
        ...
        <div class="a4-wrapper {{ $index < count($reports) - 1 ? 'page-break' : '' }}">
            ...
            <div class="header-main">
                ...
                <div class="brand-group text-right"><div class="brand-name">IRMAK TURİZM</div><div style="font-size: 10px; font-weight: 800; color: #94a3b8; letter-spacing: 3px; margin-top: 5px;">SERVISPILOT PRO</div></div>
            </div>

            <div style="display: flex; gap: 20px; margin-bottom: 30px;">
                <div class="stat-card"><div class="stat-label">PERSONEL KAYITLARI</div><div class="stat-value">{{ $driver->full_name }}</div><div style="font-size: 10px; color: #64748b; font-weight: 700; margin-top: 4px;">TC: {{ $driver->tc_no ?? '-----------' }}</div></div>
                <div class="stat-card" style="text-align: right;"><div class="stat-label">AYLIK BAZ MAAŞ</div><div class="stat-value" style="font-size: 26px;">{{ number_format($report['base_salary'], 2, ',', '.') }} ₺</div></div>
            </div>
                ...
                <table>
                    ...
                    <tbody>
                        @foreach($report['details'] as $summary)
                            <tr>
                                <td><div style="font-size: 9px; font-weight: 800; color: #64748b; margin-bottom: 2px;">{{ $summary['customer_name'] }}</div><div style="font-size: 13px; font-weight: 800; color: #2563eb;">{{ $summary['route_name'] }}</div></td>
            ...
            <div style="display: flex; gap: 30px; align-items: stretch;">
                ...
                <div class="total-card-pro">
                    <div style="display: flex; justify-content: space-between; font-size: 10px; color: #94a3b8; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 4px; margin-bottom: 6px;">
                        <span>Hakediş (Maaş+Sefer):</span>
                        <span style="color: white;">+{{ number_format($report['base_salary'] + $report['extra_earnings'], 2, ',', '.') }} ₺</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; font-size: 10px; color: #10b981; font-weight: 800; margin-bottom: 8px;">
                        <span>EKSTRA (+):</span>
            ...
            <div class="signature-grid">
                <div class="signature-item"><div class="signature-line"></div><div style="font-size: 10px; font-weight: 900; text-transform: uppercase;">İŞVEREN ONAYI</div></div>
                <div class="signature-item"><div class="signature-line"></div><div style="font-size: 10px; font-weight: 900; text-transform: uppercase;">PERSONEL: {{ $driver->full_name }}</div></div>
            </div>
        </div>
    @endforeach
</body>
