...
@section('title', 'Yeni Yakıt Kaydı')
@section('subtitle', 'Araç bazlı yakıt giriş formu')
    ...
    <div class="rounded-[32px] border border-slate-200/70 bg-white/95 shadow-[0_20px_60px_rgba(15,23,42,0.08)] overflow-hidden">
        <div class="border-b border-slate-100 bg-gradient-to-r from-slate-50 via-white to-blue-50/40 px-6 py-6">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    <h1 class="text-[26px] font-bold tracking-tight text-slate-900">Yeni Yakıt Kaydı Ekle</h1>
                    <p class="mt-1 text-sm text-slate-500">Araç, km, litre, birim fiyat ve istasyon bazlı yakıt kaydı oluştur</p>
                </div>
        ...
        <form action="{{ route('fuels.store') }}" method="POST" class="p-6 space-y-6" id="fuelCreateForm">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
                ...
                <div>
                    ...
                    <select name="fuel_type" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-800 shadow-sm outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100">
                        @foreach($fuelTypeOptions as $fuelType)
                            <option value="{{ $fuelType }}" {{ old('fuel_type', 'Dizel') === $fuelType ? 'selected' : '' }}>
                                {{ $fuelType }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="fuel_station_id" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-800 shadow-sm outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100">
                        ...
                        @foreach($stations as $station)
                            <option value="{{ $station->id }}" {{ old('fuel_station_id') == $station->id ? 'selected' : '' }}>
                                {{ $station->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text"
                           ...
                           placeholder="Kayıtlı istasyon yoksa yazılabilir"
                           class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-800 shadow-sm outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="notes"
                          rows="4"
                          class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-slate-800 shadow-sm outline-none focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100"
                          placeholder="Yakıt alımı ile ilgili açıklama...">{{ old('notes') }}</textarea>
            </div>
