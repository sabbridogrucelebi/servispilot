    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $row) {
            ...
            VehicleMaintenance::create([
                ...
                'created_by'        => auth()->id(),
                'maintenance_type'  => trim($row['bakim_turu'] ?? '') ?: 'DİĞER BAKIMLAR',
                'title'             => trim($row['bakim_adi'] ?? '') ?: 'Bakım/Onarım',
                'service_name'      => trim($row['servis_adi'] ?? '') ?: null,
                'service_date'      => $this->parseDate($row['servis_tarihi'] ?? null) ?: now()->format('Y-m-d'),
